...
def rollout_and_collect(raw_env, model, obs_normalizer, iterations, seed=SEED, print_progress=False):
    """
    Resetea raw_env(seed) y corre hasta `iterations` pasos deterministicos de PPO, recolectando
    las mismas historias por paso que run_test_agent siempre recolecto internamente. Extraccion
    pura del cuerpo del loop de run_test_agent, sin cambio de comportamiento.
    """
    ...
    for i in range(iterations):
        ...
        hist_cpu.append(cpu_total)
        hist_ram.append(ram_total)
        ...
        if print_progress:
            print(f"| {i:^6} | {activos:^7} | {action[-1]:^14.2f} | {reward:^9.2f} |")
    ...
    return {
        ...
        'hist_cpu': hist_cpu,
        'hist_ram': hist_ram,
        'hist_queue': hist_queue,
        'hist_latency': hist_latency,
        'hist_errors': hist_errors,
        'hist_workload': hist_workload,
        'hist_activos': hist_activos,
        'scaling_events': scaling_events,
        'sla_violations': sla_violations,
        'total_steps': len(hist_cpu),
    }
...
def run_test_agent(nodes=5, iterations=5000, file='testing_metrics.csv', simulated=True):
    ...
    result = rollout_and_collect(raw_env, model, obs_normalizer, iterations, seed=SEED, print_progress=True)

    hist_reward = result['hist_reward']
    hist_cpu_total = result['hist_cpu']
    hist_ram_total = result['hist_ram']
    hist_queue = result['hist_queue']
    hist_latency = result['hist_latency']
    hist_errors = result['hist_errors']
    hist_workload = result['hist_workload']
    hist_activos = result['hist_activos']
